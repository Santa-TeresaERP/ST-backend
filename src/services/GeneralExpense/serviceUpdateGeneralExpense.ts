 ...
 */
const serviceUpdateGeneralExpense = async (
  id: string,
  body: Partial<GeneralExpenseAttributes>,
) => {
  ...
  try {
    ...
    if (expense.report_id) {
      return {
        error:
          'Este gasto ya ha sido incluido en un reporte financiero y no puede ser modificado.',
      }
    }
