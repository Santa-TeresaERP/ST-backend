 ...
 */
const serviceUpdateGeneralIncome = async (
  id: string,
  body: Partial<GeneralIncomeAttributes>,
) => {
  ...
  try {
    ...
    if (income.report_id) {
      return {
        error:
          'Este ingreso ya ha sido incluido en un reporte financiero y no puede ser modificado.',
      }
    }
