...
const serviceDeleteFinancialReport = async (id: string) => {
  ...
  try {
    ...
    await GeneralIncome.update({ report_id: null }, { where: { report_id: id }, transaction });
    await GeneralExpense.update({ report_id: null }, { where: { report_id: id }, transaction });

    // Eliminar el reporte
    await report.destroy({ transaction });

    await transaction.commit();
    return { message: 'Reporte financiero eliminado y transacciones liberadas correctamente.' };

  } catch (error) {
    await transaction.rollback();
    console.error(`Error al eliminar el reporte financiero con ID ${id}:`, error);
