...
const serviceCreateFinancialReport = async (body: FinancialReportAttributes) => {
  ...
  const { start_date, end_date, observations } = validation.data;
  const transaction = await sequelize.transaction();

  try {
    const incomesToReport = await GeneralIncome.findAll({
      where: { report_id: null, date: { [Op.between]: [start_date, end_date] } },
      transaction,
    });
    ...
    const total_income = incomesToReport.reduce((sum, item) => sum + item.amount, 0);
    const total_expenses = expensesToReport.reduce((sum, item) => sum + item.amount, 0);
    ...
    if (incomeIds.length > 0) {
      await GeneralIncome.update({ report_id: newReport.id }, { where: { id: incomeIds }, transaction });
    }
    if (expenseIds.length > 0) {
      await GeneralExpense.update({ report_id: newReport.id }, { where: { id: expenseIds }, transaction });
    }
