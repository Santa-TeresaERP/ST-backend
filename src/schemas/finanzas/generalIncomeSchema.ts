...
export const createGeneralIncomeSchema = z.object({
  module_id: z.string().uuid({ message: 'El ID del módulo debe ser un UUID válido.' }),
  income_type: z.string().min(1, 'El tipo de ingreso es obligatorio.').max(100, 'El tipo no debe exceder los 100 caracteres.'),
  amount: z.number({ invalid_type_error: 'El monto debe ser un número.' }).positive('El monto debe ser un número positivo.'),
  date: z.coerce.date({
